#ifndef CAFFE_EXTERNAL_PROJECTION_LAYER_HPP_
#define CAFFE_EXTERNAL_PROJECTION_LAYER_HPP_
...
class ExternalProjectionLayer : public Layer<Dtype> {
 public:
  explicit ExternalProjectionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  ...
  virtual inline const char* type() const { return "ExternalProjection"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }//requires two bottom blobs
 ...
 protected:
  ...
  /**
   * @brief Computes the error gradient w.r.t. the projection inputs.
   *
  ...
  int M_;//bottom[0]->shape(1)
  int K_;//bottom[1]->shape(1)
  int N_;//bottom[0]->shape(0)
  int T_;//truncate
};

}  // namespace caffe

#endif  // CAFFE_EXTERNAL_PROJECTION_LAYER_HPP_
